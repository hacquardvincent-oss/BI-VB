...
| `server/trends.js` (+ `web/tendances.html`/`tendances.js`) | **Page Tendances = onglet nav « 📈 Tendances »** (analyses temps long). `GET /api/trends?url=` : séries **MENSUELLES N vs N-1** (taux de transfo, taux d'engagement, taux d'ajout panier, ajouts panier, sessions, nouveaux utilisateurs ≈ proxy base client) depuis le jeu GA `ga` (date×…) + OMS (`monthlyEShopCA` pour le TT). **Filtre URL** (comme GA4) via le nouveau jeu **`gapagedaily`** (date×pagePath ; fetcher `fetchPageDaily` câblé au refresh GA4). Front : 1 courbe/métrique, trait plein N / pointillé N-1. ⚠️ profondeur = ce qui est importé (1 an dans le slot N + N-1 dans N1). |
