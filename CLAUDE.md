...
**Structure des onglets header** : 📊 Reporting (page Full = tableaux de pilotage standard) · 💰 Analyse commerciale (zoom offre) · 🏷️ Analyse de saison (collection, période longue) · 🎯 Objectifs (mensuel + **détail jour par jour** N vs N-1 dépliable par mois, objectif quotidien réparti sur le profil N-1, `GET /api/objectives/daily?month`) · 🔮 **Anticipation** (page dédiée, saisie période N-1, cf. table fichiers) · 💬 Retours. **Anticipation N'EST PLUS une vue Reporting** (déplacée en onglet header).
