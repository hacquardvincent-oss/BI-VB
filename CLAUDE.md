...
- **Éditeur de vue WYSIWYG** : mode édition (case « Dans la vue » + ⠿ drag'n'drop sur chaque tableau). Vues **partagées**
  éditées par les admins → `/api/layouts` (table `layouts`). **Tableaux de bord PERSONNELS** par utilisateur (« ➕ Nouveau
  tableau de bord ») → `/api/myviews` (table `user_views`, 1 ligne/compte) : chacun crée/édite/supprime ses onglets
  `my:<key>`. `getLayout`/`renderReport` gèrent `my:`, `full` (= tous les tableaux), et les overrides partagés.
