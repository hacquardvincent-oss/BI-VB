...
- **Montage des routes** (`index.js`) : `/healthz`; `/auth`; `/api/ingest`; `/api/report` (**reports.js ET pdf.js**);
  `/api/ga4`; `/api/wshop`; `/api/googleads`; `/api/meta`; `/api/reco`; `/api/objectives` (+ `/daily` détail jour, `/days` objectifs au jour) ; `/api/feedback`; `/api/snapshots`; `/api/anticipation`; `/api/trends`; `/api/tables`; `/api/specs/reload` (admin);
...
| `server/tables.js` (+ `web/creation.html`/`creation.js`) | **Registre GLOBAL de tableaux réutilisables** (onglet header **🧱 Création**). Un tableau = widget « from scratch » nommé + **catégorie** (taxo data-analyse) + **format** (reporting/commerciale/saison), réutilisable dans toutes les vues. CRUD `GET/POST/PUT/DELETE /api/tables` (`requireEdit`, validation par whitelists `W_DIMS/W_METRICS/W_FORMS`, table `custom_tables` jsonb, repli RAM). Les layouts le **référencent** par `{ref:id}` (résolu côté front via `widgetEntry` ; éditer le tableau dans Création se **répercute partout**). `layouts.js`/`userviews.js` `cleanCard` acceptent `{ref}`. Front (`app.js`) : **pop-in « 📋 Choisir les tableaux »** (`openTableSelector`) = liste compacte par **catégorie + format + recherche**, cases à cocher add/remove ; ouverte au clic ✏️ Éditer. **Mode édition = vue COURTE** (seulement les tableaux inclus + ✕ Retirer + ⠿ drag), fini la longue page « Full ». `CAT_META`/`CAT_ORDER`/`THEME_CAT`/`CARD_FORMAT` mappent thèmes→catégories. La page Création liste **TOUT** : tableaux du registre **rendus en aperçu (données d'exemple, Chart.js, `widgetCardHtml`/`drawWidget`/`genData`/`SAMPLE`)** + **aperçu LIVE** du tableau en cours de composition (`updatePreview`) + cartes natives listées (référence, `nativeCards()`). Pop-in : **modèles 1 clic** (`VIEW_TEMPLATES`), **favoris** épinglés (localStorage), **badge + filtre « données disponibles »** (`CARD_NEEDS`/`needOf`/`dataAvailable`). Barre d'édition : **⧉ Dupliquer** la vue (`duplicateView`) ; nouveau dashboard démarre quasi vierge (`['kpi']`) + pop-in ouverte. |
| `server/trends.js` (+ `web/tendances.html`/`tendances.js`) | **Page Tendances = onglet nav « 📈 Tendances »** (analyses temps long ; **layout 2 zones** = panneau gauche `#setupCol` : période N + N-1 en **1 calendrier range chacun** [flatpickr] + filtre URL + chargement OMS/GA4/Ads/Meta/Y2 liés aux périodes). `GET /api/trends?url=` : séries **MENSUELLES N vs N-1** : taux de transfo, taux d'engagement, taux d'ajout panier, **taux panier→achat** (funnel), **panier moyen, indice de vente, taux de retour + retours €/nb** (OMS + `ret`), **alertes stock dans le temps** (jeu daté `bisdaily` produit par `refreshStockAlerts`), **part de nouveaux** (fidélité), **ROAS / CPA / dépense Ads** (`ads`), sessions, nouveaux utilisateurs. Ordre cartes : **KPI eshop + acquisition AVANT marketplace** puis cohortes. **CA par marketplace dans le temps** (`calc.marketplaceMonthly`, règles figées GL=674SFS) → carte multi-lignes. **Cohortes de réachat** (`calc.cohortRetention` : réachat 30/60/90 j par mois de 1ʳᵉ commande, OMS N+N-1, périmètre EShop). **Filtre URL** (comme GA4) via le jeu **`gapagedaily`** (date×pagePath ; `fetchPageDaily`). `monthOf` parse GA `YYYYMMDD`/ISO/FR. ⚠️ profondeur = ce qui est importé ; cohortes nécessitent la colonne `Client` (import complet WSHOP). |
...
| `server/anticipation.js` (+ `web/anticipation.html`/`anticipation.js`) | **Page Prévisionnel = onglet HEADER** (ex-« Anticipation », pas une vue Reporting ; fichiers/route `anticipation` conservés). **Layout 2 zones** (`.wrap.layout-side.no-rightnav`) : panneau gauche `#setupCol` (1·Période N-1, 2·Chargement) + `#mainCol`. L'utilisateur saisit une **période N-1** → `GET /api/anticipation?from&to` : **Synthèse 360** (CA/KPI, full/off, démarque), **détail par SEMAINE** (`calc.weeklyHistory` : CA/full-off/dates + top familles/produits par semaine + **dates d'opération** = jours où la démarque domine ≥50 %), **top familles & produits cumul**, **CRM & acquisition par semaine** (`weeklyChannels` depuis `ga`, `weeklyCampaigns` depuis `gacampdaily`), jours pics, **playbook**, Google/Meta Ads (`calc.calcAds`), projetés sur la **période N équivalente (+364 j)**. **4 axes data-analyse** (quick wins sur données déjà ingérées) : **Stock & alertes** (demande back-in-stock `bis` par produit = réassort prioritaire), **Campagnes→produits** (`gacampcat` familles portées + `gacampaignland` campagne→landing→achats), **Calendrier média hebdo** (`ads` date×campagne → coût/ROAS/CPA par semaine + flag fatigue créative), **CRM & top pages** (`emailPeakHour`, top campagnes CRM via `gacampdaily`, nouveaux vs récurrents `gacampnr`, top landing `galanding`/pages `gapages`). Scanne les slots N/N1, dégradation gracieuse. `calc.buildAnticipation` + `calc.weeklyHistory` acceptent une **fenêtre explicite `{from,to}`**. **Panneau de chargement** (réutilise les connecteurs/slots partagés, **lié à la période sélectionnée**) : WSHOP = `/refresh?from&to&cfrom&cto` (période N-1 + année avant) ; GA4/Ads/Meta = `/refresh?from&to`. Boutons affichés selon `/status`. |
| `server/periodic.js` (+ `web/periodique.html`/`periodique.js`) | **Reporting périodique MULTI-RÉSOLUTION = onglet nav « 📅 Périodique »** (socle des reportings quotidien/hebdo cibles). `GET /api/periodic?asof` : depuis une **date d'arrêté**, dérive 4 fenêtres (`calc.deriveWindows` : **Jour / Semaine WTD / Mois MTD / Saison-à-date** ; saison E fév-juil, H août-janv) + leur N-1 (−364 j), et calcule pour chacune le **bloc KPI** (`calc.kpiBundle` : Global/FR/Inter/Full/Démarque × CA/Cmd/Pièces/IV/PM/Sessions/TT, périmètre EShop). Meilleur jeu OMS couvrant chaque fenêtre (oms/saisonoms), sessions via gatot/gasess. Front : presets 🌅 Quotidien / 📅 Hebdo, blocs empilés N vs N-1. ⏳ À venir : détail jour-par-jour MTD + objectifs Full/Dém + sessions FR/Inter. |
| `web/databar.js` | **Panneau de CHARGEMENT DES DONNÉES commun** à toutes les briques (`initDataBar({getPeriods, onLoaded, title})` rendu dans `<div id="dataBar">`). Récap « 📦 déjà en mémoire » (`/api/ingest/status`, partagé/persisté), refresh par connecteur (WSHOP complet + ⚡ delta, GA4/Ads/Meta/Y2) avec **skip si déjà couvert** (date_min≤from≤to≤date_max → pas de retéléchargement) + bouton **🔔 Stock & alertes stock** (`/saison-merch?slot=` → jeux STANDARDS `stock`/`bis`, analysables partout — `refreshSaisonMerch` paramétré par `slot`) + **import de fichier** (oms, ret, **bis**, ga, ads, y2, ref, impl, offre). L'import OMS complet charge déjà retours (`ret`/`retprod`) + alertes (`bis`). Options : `slot` (ex. `'saison'` → import WSHOP `&slot=saison`, récap des jeux `saison*`), `connectors` (filtre les boutons). **Câblé sur toutes les briques** : Tendances, Prévisionnel, Commerciale (ajout non destructif), Objectifs (passée en 2 zones), Saison (récap seul, `slot:'saison'`, ses imports dédiés conservés). Reporting garde son panneau riche natif. |
...
**Clé client cohortes (ADR-009)** : colonne **`Client`** = **hash SHA-256 tronqué** de l'email/identifiant (`wshop.clientKey`, jamais l'email en clair, non réversible) → relie les commandes d'un même client pour `calc.cohortRetention` (réachat). Alias `client` dans `OMS_ALIASES`. 19ᵉ colonne d'`OMS_HDRS`. ⚠️ Pour les **uploads**, la colonne email reste jetée (pas encore hashée à l'upload) → cohortes = WSHOP pour l'instant. Exige un **import complet WSHOP** pour peupler `Client`.
...
### Modules / thèmes
`MODULE_ORDER` (barre de vues, filtrée RBAC). Thèmes `THEME_ORDER = [P,PA,CO,T,ES,AN,SK,OS,AQ,IN,MP,CR,OF,Z]`
(`THEME_META` = bannières ; **AN** = 🚫 Annulations & Remboursements [cartes `annulations`+`retours`], **SK** = 🔔 Alertes stock [`stockalerts`]).
**Profils de cadence** (le `preset` du module est appliqué au changement de vue via `switchModule` → clic du raccourci `[data-range]` correspondant ; ignoré si `m.dates` ou preset sans raccourci `all`/`today`) : **`dailysoft`** (🌅 Matin, preset `yesterday` : kpi+cumul+timeline+produits), **`hebdo`** (📅 Hebdo & cumul mois, preset `week` : kpi+cumul+familles+produits+daily+channels+annulations+retours). `direction` porte la carte `cumul`.
**Structure des onglets header** : nav gauche = **types d'analyse** (Reporting, Analyse commerciale, Analyse de saison, Objectifs, Prévisionnel, **📈 Tendances**) ; **CTA droite (à côté d'Admin)** = utilitaires **🧱 Création** + **💬 Retours** (séparés des types de reporting). 📊 Reporting (page Full = tableaux de pilotage standard) · 💰 Analyse commerciale (zoom offre) · 🏷️ Analyse de saison (collection, période longue) · 🎯 Objectifs (mensuel + **détail jour par jour** N vs N-1 dépliable par mois, objectif quotidien réparti sur le profil N-1, `GET /api/objectives/daily?month`) · 🔮 **Prévisionnel** (ex-Anticipation ; page dédiée 2 zones, saisie période N-1 + chargement données lié à la période, synthèse 360 + détail hebdo full/off + CRM/acquisition/familles/produits par semaine, cf. table fichiers) · 💬 Retours. **Prévisionnel N'EST PLUS une vue Reporting** (onglet header).
...
- **Widgets « from scratch »** (🧱 Nouveau tableau, en mode édition) : l'utilisateur compose **Donnée × Métrique × Forme**
  (KPI/tableau/barres/donut/courbe) + **Top N libre (1–50)** + toggle N-1. Widget = objet `{id,title,dim,metric,form,top,n1}`
  mêlé aux clés string dans le layout (validé serveur par whitelists `W_DIMS/W_METRICS/W_FORMS` dans layouts.js & userviews.js
  ; `top` clampé 1–50 côté serveur → Top N libre OK sans modif). Front : catalogue `W_DIMS`/`W_METRICS`, `widgetData(w,rep)`
  extrait depuis l'objet `rep` (source unique, AUCUN nouveau endpoint), `renderCustomWidget` + `renderWidgetCharts`
  (file `W_PENDING`, registre `W_CHARTS`). **Édition en place** (palier 1) : `openWidgetBuilder(cb, existing)` rouvre le volet
  **pré-rempli** ; bouton **⚙️ Modifier** (`.edit-cfg`) sur chaque widget en mode édition → remplace le widget (même `id`)
  par `editWrapHtml` + re-`renderWidgetCharts`. Métriques enrichies : `caEnt`/`caSFS` (omnicanal). ⏳ À venir : éditer les
  cartes natives en place (palier 2) ; widgets croisés 2 dimensions (palier 3).
