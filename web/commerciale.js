...
function renderAll(rep, day) {
  ...
  const c = rep.ca.n;
  if (c && (c.caFP > 0 || c.caOP > 0)) mk('opDonut', { type: 'doughnut', data: { labels: ['Full Price', 'Off Price'], datasets: [{ data: [Math.round(c.caFP), Math.round(c.caOP)], backgroundColor: ['#6E7B8B', '#C58BA3'], borderColor: '#FFFFFF', borderWidth: 2 }] }, options: { responsive: true, maintainAspectRatio: false, cutout: '58%', plugins: { legend: { position: 'bottom', labels: { color: '#9CA1AB', font: { size: 9 }, boxWidth: 10, padding: 6, usePointStyle: true } } } } });
