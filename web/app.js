...
const MODULES = {
  ...
  ws_comptes: {
    ...
    layout: ['ws_cockpit', 'ws_matrix', 'marketplace', 'mpfamilles'],
  },
  ws_arbitrage: {
...
const THEME_OF = {
  ...
  dir_overview: 'HE', retail_parc: 'HE', retail_cockpit: 'HE', ws_matrix: 'HE', ws_cockpit: 'HE', achats_otb: 'HE', achats_saison: 'HE', fin_bridge: 'HE', col_perf: 'HE', // cartes métier (démo)
  campaignland: 'CR',
...
const CARD_LABELS = {
  ...
  marketplace: 'CA Marketplace', crosschannel: 'Cross-canal', mpfamilles: 'Top familles / marketplace', retailstores: 'Réseau boutiques', pnl: 'P&L & marge', dir_overview: 'Vue d\'ensemble groupe (démo)', retail_parc: 'Parc magasin (démo)', retail_cockpit: 'Cockpit magasin (démo)', ws_matrix: 'Qui vend quoi (démo)', ws_cockpit: 'Cockpit wholesale (démo)', achats_otb: 'Open-to-Buy (démo)', achats_saison: 'Bilan saison (démo)', fin_bridge: 'Budget & marge (démo)', col_perf: 'Performance collection (démo)', campaignland: 'Campagne → landing', pagesrc: 'Source → page',
...
function renderReport(rep) {
  ...
  if (cx) {
    ...
    cancellationsCard = `<div class="card"><h3>⛔ Annulations EShop — commandes annulées (source OMS / WSHOP)</h3><div class="kgrid">${tiles}</div>${detailHtml}<div class="note"><b>Taux du scorecard</b> = <b>CA non livré total ÷ CA EShop</b> = (${fEur(cx.caNonLivre != null ? cx.caNonLivre : cx.caAnnuleEstime)} annulé + ${fEur(cx.caIncomplete || 0)} expédié incomplet) ÷ CA EShop — comme le TCD de référence, qui additionne les lignes <b>Quantité non livré &gt; 0</b> (Annulée <b>et</b> Expédiée Incomplète). Autres lectures (hors marketplace) : <b>commande</b> = commandes annulées ÷ commandes · <b>pièces</b> = ${fInt(cx.qteAnnulee)} pièces non livrées ÷ pièces commandées. Si tu comptes en <b>pièces</b> (ton TCD), regarde la tuile « Taux annul. (pièces) ». <b>Annulations</b> = statut <b>Annulée</b> (Cancelled). Les <b>expéditions incomplètes</b> (ShippedIncomplete) sont comptées <b>à part</b> et n'entrent pas dans le taux. ⚠️ Couleur inversée. ℹ️ Statut WSHOP <b>live</b> ≠ export OMS figé. À ne pas confondre avec les retours.</div></div>`;
  }
  ...
  const campaignsCard = campRows ? `<div class="card"><h3>Campagnes (UTM) — N vs N-1</h3>${gaStaleWarn(rep)}
...
function renderScorecard(title, pack, showDetails, sig) {
  ...
  // Taux d'annulation du scorecard = CA NON LIVRÉ TOTAL ÷ CA EShop, croisé sur le CA comme le report de
  // référence (TCD) : le CA non livré additionne les commandes ANNULÉES *et* les expéditions INCOMPLÈTES
  // (toute ligne avec Quantité non livré > 0). Le détail annulée / incomplète reste dans la carte Annulations.
  const annCA = (c, kk) => (c && kk && kk.ca > 0) ? ((c.caNonLivre || 0) + (c.caIncomplete || 0)) / kk.ca : null;
...
function renderCharts(rep) {
  ...
  if (rep.ca && rep.ca.n) {
    ...
    if (seg.length) mk('caDonut', { type: 'doughnut', data: { labels: seg.map(x => x[0]), datasets: [{ data: seg.map(x => Math.round(x[1])), backgroundColor: PALETTE, borderColor: '#FFFFFF', borderWidth: 2 }] }, options: window.pieOutOpts ? window.pieOutOpts(fEur) : donutOpts });
  }
